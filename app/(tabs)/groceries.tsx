...
export default function GroceriesScreen() {
  ...
  const { groceryItems, addGroceryItem, removeGroceryItem } = usePantry();
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.lead}>
        Add groceries manually with the plus button. Voice input can be added in a later step.
      </Text>

      {groceryItems.length === 0 ? (
        <View style={styles.empty} lightColor="#f4f4f5" darkColor="#27272a">
          <FontAwesome name="shopping-cart" size={40} color={palette.tabIconDefault} />
          ...
          <Text style={styles.emptySubtitle} lightColor="#52525b" darkColor="#a1a1aa">
            Add missing ingredients from Recipes or use the plus button here.
          </Text>
        </View>
      ) : (
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.row} lightColor="transparent" darkColor="transparent">
              <View style={styles.rowMain}>
                <FontAwesome name="check-square-o" size={17} color={palette.tint} style={styles.rowIcon} />
